...
| 里程碑 | 状态 | commit | 验收 |
|---|---|---|---|
| M0 版本化迁移 + M1 原创发布/连载 | ✅ 完成 | `125d9e5` + `c42699e` | 5/5 通过（短篇/连载全流程/越权 403/删除保护/迁移无影响） |
| M2 订阅 + 追更提醒 | ✅ 完成 | `1162710` | 5/5 通过（追更通知/防风暴/退订/可见性/MCP 一致） |
| M3 作者反馈面板 | ✅ 完成 | `acb203e` | 集成 18/18 + 单测 27/27 |

**实施中补充/修正**（超出 spec 原稿，已合并进代码）：
- 书架列表（`list_books`）补 `kind`/`series_id`/`updated_at` 字段，可区分原创与连载章节
- `notifySubscribers` 返回**实际新通知数**（防风暴验证可靠）
- `getWorkBook` 补 `view_count`、`recent_reviews` 补 `content` 字段
